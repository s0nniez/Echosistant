...
def mainProfilePage() {	
    dynamicPage(name: "mainProfilePage", title:"", install: true, uninstall: installed) {
        ...
        section("Feedback and Control") {
            href "feedback", title: "Feedback and Device Control Groups"//, description: pSendComplete(), state: pSendSettings()   
        }
		section ("Shortcuts") {
        	href "Shortcuts", title: "Profile Shortcuts"
        }    
		section ("Task Tracking") {
            href "pTrackers", title: "Task Tracking"//, description: pSendComplete(), state: pSendSettings()    
        }
...
def Shortcuts(){
    dynamicPage(name: "Shortcuts", title: "", uninstall: false){    
        section("Create and View Shortcuts") {
            app(name: "Shortcuts", appName: "Shortcuts Profile", namespace: "Echo", title: "New Child App", multiple: true)  
        }
    }
}    
...
def feedback(){
    dynamicPage(name: "feedback", title: "", uninstall: false){  
        section("") {
            ...
            href "fDevices", title: "Device Feedback"//, description: pRestrictComplete(), state: pRestrictSettings()
...
def pSend(){
  ...
  //          input "EchoDevice", "ask.ID", title: "Choose Alexa Device"
...
def profileFeedbackEvaluate(params) {
    ...
    if (parent.debug) log.debug "sendNotificationEvent sent to CoRE from ${app.label}"
       ...
       /// CHECK FOR MOTION
            ...
            if(tts.contains("motion") || tts=="is there anyone" || tts=="is anyone" || tts=="is something moving" || tts.contains("is someone")) { 
            	if(fMotion != null) {
                def devListMotion = []
            ...
            //  PRESENCE FEEDBACK
            if (tts.startsWith("is anyone") || tts.startsWith("who") || tts.contains("people") || tts == ("check on my family") || tts.startsWith("how")) {
                if (deviceType == "fbPresence") {
                    ...
                    if(fPresence){
                        ...
                        if (tts=="how many people are at home" || tts=="how many family members are at home" || tts.contains("is anyone")) { //&& tts.endsWith("home")) {
                                if (devListP.size() == 1) {
            ...
            if (tts.contains("smart home monitor") || tts.contains("alarm system") || tts.contains("alarm") || tts.contains("security")){
                def sSHM = location.currentState("alarmSystemStatus").value      
                sSHM = sSHM == "off" ? "Disarmed" : sSHM == "away" ? "Armed Away" : sSHM == "stay" ? "Armed Home" : "unknown"
                outputTxt = "Your Smart Home Monitor Status is currently " +  sSHM
                return ["outputTxt":outputTxt, "pContCmds":state.pContCmds, "pShort":state.pShort, "pContCmdsR":state.pContCmdsR, "pTryAgain":state.pTryAgain, "pPIN":pPIN]
            }
            ...
            if (tts.contains("window") || tts.contains("vent") || tts.contains("lock") || tts.contains("blind") || tts.contains("curtain") || tts.contains("shade") || tts.contains("door") || tts.contains("light") || tts.contains("fan")) {
                if (tts.contains("how") || tts.contains("on") || tts.endsWith("off") || tts.contains("open") || tts.contains("closed") || tts.startsWith("check")) {
                    def devList = [] 
                    ...
                    if (tts.startsWith("are") || tts.startsWith("is")) {
                        if (devList.size() > 0) {
                            ...
                            else {outputTxt = "Yes, there are " + devList.size() + " " + fName + " " + command + " in the ${app.label} "
                                 }
                        }    
...
private getFeedbackCommand(text){
    ...
    // Batteries
    if (text.contains("battery") || text.contains("batteries")) {
    	command = "level"
        deviceType = "fbBattery"
    }    
...
//LIGHT SWITCHES
        ...
        if (gSwitches) {
            ...
            if (command == "undefined") {
               	command = text.contains("darker") ? "decrease" : text.contains("too bright")  ? "decrease" : text.contains("dim") ? "decrease" : text.contains("dimmer") ? "decrease" : text.contains("lower") ? "decrease" :"undefined"
            	}
            if (command == "undefined") {
...
// SC-TWO ACTIONS HANDLER
def scTwoActions(){
    if (sc2Switches) {
    	if (sc2SwitchesCmd == "on") {sc2Switches.on()}
        	else if (sc2SwitchesCmd == "off") {sc2Switches.off()}
            }
	if (sc2Bulbs) {
        processColor()
        }
	if (sc2Dimmers) {
		def level = sc2DimmersLevel < 0 || !sc2DimmersLevel ?  0 : sc2DimmersLevel >100 ? 100 : sc2DimmersLevel as int
            sc2Dimmers.setLevel(level) 
            }		
    if (sc2Garage) {        
        if (sc2GarageCmd == "open") {sc2Garage.open()}
       	if (sc2GarageCmd == "close") {sc2Garage.close()}
        }
    if (sc2Fans) {        
        if (sc2FansCmd == "on") {sc2Fans.on()}
       	if (sc2FansCmd == "off") {sc2Fans.off()}
        if (sc2FansCmd == "low") {sc2Fans.setLevel(33)}
        if (sc2FansCmd == "med") {sc2Fans.setLevel(66)}
        if (sc2FansCmd == "high") {sc2Fans.setLevel(99)}
            }
    if (sc2Vents) {        
        if (sc2VentsCmd == "open") {sc2Vents.on()}
       	if (sc2VentsCmd == "close") {sc2Vents.off()}
        if (sc2VentsCmd == "25") {sc2Vents.setLevel(25)}
        if (sc2VentsCmd == "50") {sc2Vents.setLevel(50)}
        if (sc2VentsCmd == "75") {sc2Vents.setLevel(75)}
            }
    if (sc2Shades) {        
        if (sc2ShadesCmd == "open") {sc2Shades.setLevel(100)}
       	if (sc2ShadesCmd == "close") {sc2Shades.setLevel(0)}
        if (sc2ShadesCmd == "25") {sc2Shades.setLevel(25)}
        if (sc2ShadesCmd == "50") {sc2Shades.setLevel(50)}
        if (sc2ShadesCmd == "75") {sc2Shades.setLevel(75)}
            }
    if (sc2Locks) {        
        if (sc2LocksCmd == "lock") {sc2Locks.lock()}
       	if (sc2LocksCmd == "unlock") {sc2Locks.unlock()}
        }
    if (sc2Modes) {
    	setLocationMode(sc2Modes)
        }
}
// SC-THREE ACTIONS HANDLER
def scThreeActions(){
    if (sc3Switches) {
    	if (sc3SwitchesCmd == "on") {sc3Switches.on()}
        	else if (sc3SwitchesCmd == "off") {sc3Switches.off()}
            }
	if (sc3Bulbs) {
        processColor()
    	}
	if (sc3Dimmers) {
		def level = sc3DimmersLevel < 0 || !sc3DimmersLevel ?  0 : sc3DimmersLevel >100 ? 100 : sc3DimmersLevel as int
            sc3Dimmers?.setLevel(level) 
            }		
    if (sc3Garage) {        
        if (sc3GarageCmd == "open") {sc3Garage.open()}
       	if (sc3GarageCmd == "close") {sc3Garage.close()}
        }
    if (sc3Fans) {        
        if (sc3FansCmd == "on") {sc3Fans.on()}
       	if (sc3FansCmd == "off") {sc3Fans.off()}
        if (sc3FansCmd == "low") {sc3Fans.setLevel(33)}
        if (sc3FansCmd == "med") {sc3Fans.setLevel(66)}
        if (sc3FansCmd == "high") {sc3Fans.setLevel(99)}
            }
    if (sc3Vents) {        
        if (sc3VentsCmd == "open") {sc3Vents.on()}
       	if (sc3VentsCmd == "close") {sc3Vents.off()}
        if (sc3VentsCmd == "25") {sc3Vents.setLevel(25)}
        if (sc3VentsCmd == "50") {sc3Vents.setLevel(50)}
        if (sc3VentsCmd == "75") {sc3Vents.setLevel(75)}
            }
    if (sc3Shades) {        
        if (sc3ShadesCmd == "open") {sc3Shades.setLevel(100)}
       	if (sc3ShadesCmd == "close") {sc3Shades.setLevel(0)}
        if (sc3ShadesCmd == "25") {sc3Shades.setLevel(25)}
        if (sc3ShadesCmd == "50") {sc3Shades.setLevel(50)}
        if (sc3ShadesCmd == "75") {sc3Shades.setLevel(75)}
            }
    if (sc3Locks) {        
        if (sc3LocksCmd == "lock") {sc3Locks.lock()}
       	if (sc3LocksCmd == "unlock") {sc3Locks.unlock()}
        }
    if (sc3Modes) {
    	setLocationMode(sc3Modes)
        }
}*/
/************************************************************************************************************
