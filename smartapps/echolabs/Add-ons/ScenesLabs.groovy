...
def sceneSetup() {
           dynamicPage(name: "sceneSetup", title: "", install: true, uninstall: true,submitOnChange: true) {
            	section ("Run Scene Automatically"){
                    input "sRun", "bool", title: "Execute this Scene when Profile Runs?", default: false, submitOnChange: true
                 }
                 section ("Switches", hideWhenEmpty: true){
                    input "sSwitches", "capability.switch", title: "Select Lights and Switches...", multiple: true, required: false, submitOnChange: true
                        if (sSwitches && sRun) input "sSwitchCmd", "enum", title: "Command To Send when the Profile Runs (leave blank if you don't want this scene to run automatically)",
                	...
                	input "cFan", "capability.switchLevel", title: "Select These Fan(s)...", multiple: true, required: false
                }
                section ("Dimmers", hideWhenEmpty: true){
                    input "sDimmers", "capability.switchLevel", title: "Select Dimmers...", multiple: true, required: false , submitOnChange:true
                        if (sDimmers && sRun) input "sDimmersCmd", "enum", title: "Command To Send when the Profile Runs (leave blank if you don't want this scene to run automatically)",
				...
				section ("Colored lights", hideWhenEmpty: true){
            		input "sHues", "capability.colorControl", title: "Select These Colored Lights...", multiple: true, required: false, submitOnChange:true
            			if (sHues && sRun) {
                        	input "sHuesCmd", "enum", title: "Command To Send when the Profile Runs (leave blank if you don't want this scene to run automatically) ", 
                ...
                section ("Climate Control", hideWhenEmpty: true){ 
                 	input "sTstat", "capability.thermostat", title: "Allow These Thermostat(s)...", multiple: true, required: false
                    input "sRoomTemp", "capability.temperatureMeasurement", title: "Allow These Device(s) to Report the Room Temperature...", multiple: true, required: false
                    input "sVent", "capability.switchLevel", title: "Allow These Smart Vent(s)...", multiple: true, required: false
                }                
                section ("Sensors", hideWhenEmpty: true) {
                 	input "sMotion", "capability.motionSensor", title: "Allow These Motion Sensor(s)...", multiple: true, required: false
                    input "sContact", "capability.contactSensor", title: "Allow These Contact Sensor(s)...", multiple: true, required: false      
            		input "sWater", "capability.waterSensor", title: "Allow These Water Sensor(s)...", multiple: true, required: false                       
                }
                section ("Media" , hideWhenEmpty: true){
                    input "sSpeaker", "capability.musicPlayer", title: "Allow These Media Player Type Device(s)...", required: false, multiple: true
                    input "sSynth", "capability.speechSynthesis", title: "Allow These Speech Synthesis Capable Device(s)", multiple: true, required: false
                    input "sMedia", "capability.mediaController", title: "Allow These Media Controller(s)", multiple: true, required: false
                } 
			if (sRun) {
                section ("Flash These Switches", hideWhenEmpty: true) {
                    input "sFlash", "capability.switch", title: "Flash Switch(es) when the Profile Runs ", multiple: true, required: false, submitOnChange:true
                    if (sFlash) {
                ...
                section ("Mode", hideWhenEmpty: true){
                    input "sMode", "enum", title: "Select a Location Mode to Change to, when the Profile Runs", options: location.modes.name.sort(), multiple: false, required: false, submitOnChange: true
                section ("Routine", hideWhenEmpty: true){
                    def actions = location.helloHome?.getPhrases()*.label 
                		if (actions) {
                    		actions.sort()
            			}                
                	input "runRoutine", "enum", title: "Select a Routine(s) to execute When the Profile Runs", required: false, options: actions, multiple: true
                	}
                }
            }
